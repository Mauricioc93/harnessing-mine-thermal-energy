...
def create_energysystem(consumers,case=None, generic_comp = True, **kwargs):
    ...
    for consumer in consumers:
        ...
        cons.extend_connection('IN_P_el_ind') # electricity input
        #connection to busses
        conn['el_cons_Bus'].append(cons.IN_P_el)
        conn['el_ind_Bus'].append(cons.IN_P_el_ind)
        # conn['gas_Bus'].append(cons.IN_P_gas)
        conn['mw_sup_out_Bus'].append(cons.IN_mflow_a_bypass)
        conn['mw_ret_in_Bus'].append(cons.OUT_mflow_a_bypass)
        ...
        for subcomp in cons.components:
            ...
            if 'CenHP' in subcomp.label:
                HP_cons_t_in_a = subcomp.__getitem__('t_in_a')
                HP_cons_t_out_bypass = subcomp.__getitem__('t_out_bypass')
                HP_op = subcomp.__getitem__('b_h')
                HP_b = subcomp.__getitem__('b_build')
            names[subcomp.label.replace(label, '')] = subcomp.label
                

        ES.add_eq_constraint(inflow_pipe['t_out'],HP_cons_t_in_a, 'inflow_temp_eq_out_flow_pipe')
        ES.add_eq_constraint(return_pipe['t_in'],HP_cons_t_out_bypass, 'outflow_temp_eq_out_flow_pipe')
...
def run_model(ES, operational_calc=False, case = None, typ_days=None, interface = 'gurobi', **kwargs):
    ...
    # efficiencies
    ashp_efficiency = 0.4 # turned off
    boiler_eff = 0.1
    el_heat_eff = 0.1
   ...
   # Consumer settings
    ...
    for consumer_group in consumer_groups:
        ...
        #CEN heat pump
        params[f'CenHP_C_{consumer_group}_p_spec'] = cen_wshp_costs['variable_cost']  
        params[f'CenHP_C_{consumer_group}_p_fix'] = cen_wshp_costs['nominal_cost']  
        params[f'CenHP_C_{consumer_group}_p_main'] = cen_wshp_costs['maintenance_cost'] # maintenance costs are a 2.5% of the total investment cost, check components.py
        params[f'CenHP_C_{consumer_group}_eta_h'] = wshp_efficiency
        params[f'CenHP_C_{consumer_group}_cp'] = 4.184
        #DECEN heat ex. costs
        params[f'CenHP_C_{consumer_group}_n'] = n_con
        params[f'CenHP_C_{consumer_group}_he_p_spec'] = heat_ex_costs['variable_cost']  
        params[f'CenHP_C_{consumer_group}_he_p_fix'] = heat_ex_costs['nominal_cost'] 
        params[f'CenHP_C_{consumer_group}_he_p_main'] = heat_ex_costs['maintenance_cost']
        # BES settings
        #heat load
