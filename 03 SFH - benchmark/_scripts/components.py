...
class NetworkPipe(Component):
    ...
    def __init__(self, label, type = 'simple',n_seg = 10):
        ...
        # self.add_input(identifier='IN_t', expr = t_in)
        # self.add_input(identifier='OUT_t', expr = t_out)

        if type == 'simple':
            self.add_le_constraint(t_out,t_in,'constant_t')
        
        elif type == 'simple_losses':
            self.add_eq_constraint(m_max*0.723*cp * 1000 * (t_in - t_out),
                    (t_in - t_gr) * length * U,
                    'pipe_losses')
        elif type == 'linearized_losses':
            self.add_eq_constraint(m_max*0.723*cp * 1000 * (t_in - t_out),
                    (t_in - t_gr) * length * U,
                    'pipe_losses')
        ...
        for i in range(n_seg): #linearization of massflow - power dependance params a and b must be updated accordingly after model instance generation
            ...
            exec('self.add_le_constraint((m_flow*%s+%s)*length,p_pump,f"power_pump_%s_%s")' %(a,b,str(i),str(n_seg)))
...
class CenHeatPump(Component):
    ...
    def __init__(self,
        ...
        cp = self.make_parameter('cp', 4.184)
        n = self.make_parameter('n', 1)
        pinch_dt = self.make_parameter('pinch_dt',2.0) ### Internal HP temperatures
        dT_a_h = self.make_parameter('dT_a_h',10.0)
        # dt_b_h = self.make_parameter('dT_b_h',20.0)
        ...
        b_build = self.make_design_variable('b_build', domain=BINARY,
                                            bounds=(0, 1))
        # t_out_b = self.make_design_variable('t_out_b', bounds=(273.15,273.15+100))
        t_out_b = self.make_operational_variable('t_out_b', bounds=(273.15,273.15+100))
        self.add_le_constraint(qdot_design, self.Q_max * b_build, 'HP_Q_max')
        self.add_le_constraint(t_out_b, t_out_b_max * b_build, 'HP_T_max')
        ...
        if True:
            ...
            mflow_a = self.make_operational_variable('mflow_a',
                                                        bounds=(0, self.mdot_max))
            self.add_le_constraint(mflow_a, b_h * self.mdot_max,
                                'HP_mflow_a_op')
            ...
            mflow_b = self.make_operational_variable('mflow_b',
                                                        bounds=(0, self.mdot_max))
            self.add_le_constraint(mflow_b, b_h * self.mdot_max,
                                'HP_mflow_b_op')
            self.add_le_constraint(mflow_b, b_build * self.mdot_max,
                                'HP_mflow_b_des')
            self.add_input(identifier='IN_mflow_b', expr=mflow_b * n)
            ...
            self.add_eq_constraint(qdot_a + p_hp_h, qdot_b_h, 'energy_balance_h')
...
class DecenHeatPump(Component):
    ...
    def __init__(self, label,
                 t_out_b_h,t_out_b_c,b_connect = None):
        super().__init__(label)
        #######################################################################
        # Parameters
        #######################################################################
        ...
        if True:
            ...
            self.add_eq_constraint(p_hp_c * t_evap_c * eta_nom_c, 
                                (t_cond_c - t_evap_c) * qdot_b_c,
                                'input_output_relation_cooling')
...
class CenHeatExchanger(Component):
    ...
    def __init__(self, label, t_out_b):
        ...
        cp = self.make_parameter('cp', 4.184)
        n = self.make_parameter('n', 1)
        dt_a = self.make_parameter('dt_a', 10)
        pinch_dt = self.make_parameter('pinch_dt',3) ### Internal HP temperatures
        dT_a_h = self.make_parameter('dT_a_h',10.0)
        #######################################################################
        # Design Variables
        #######################################################################
...
class DecenHeatExchanger(Component):
    ...
    def __init__(self, label, t_out_b, b_build_hp=None):
        ...
        n = self.make_parameter('n', 1)
        dT_a = self.make_parameter('dt_a', 10)
        pinch_dt = self.make_parameter('dt_pinch',4)
...
class LinkingComponentConsumer(System):
    ...
    def __init__(self, label,
        ...
        hp = DecenHeatPump(f'HP_{label}',
            t_out_b_h=t_out_b_h,t_out_b_c=t_out_b_c)

        hx = DecenHeatExchanger(f'HX_{label}', b_build_hp=hp['b_build'],
            t_out_b=t_out_b_h)
...
class Consumer(System):
    ...
    def __init__(self, label,**kwargs):
        ...
        ashp = ASHP(f'ASHP_{label}',
        t_out_b_h=building_h['T_flow'],t_out_b_c=building_c['T_flow'], mode = ashp_mode)

                                    
        for comp in [hs_el, hs_gas, building_h,building_c, ashp]:
            self.add(comp)
        ...
        self.connect('qdot_to_bes_c', [ashp.OUT_qdot_c,
                                    building_c.IN_qdot])

        self.connect('IN_P_el', [hs_el.IN_P,
                                ashp.IN_P_c,
                                ashp.IN_P_h,
                                 ])
...
class LinkingComponentSource(System):
    """Create a linking component, modeled as a COMANDO system.
    ...
    def __init__(self, label, t_in_a, t_out_a,
        ...
        super().__init__(label)
        hp = CenHeatPump(f'HP_{label}', b_connect=b_connect,
