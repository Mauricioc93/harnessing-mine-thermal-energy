...
class FreeMassflowSource(Component):
    ...
    def __init__(self, label):
        ...
        # Electricity consumption - model is simplified with a T_price (overall price)
        # P_el = self.make_operational_variable('P_el', bounds = (0,max_Pel)) 
...
class CenHeatPump(Component):
    ...
    def __init__(self,
        ...
        self.add_le_constraint(qdot_design, self.Q_max * b_build, 'HP_Q_max')
        self.add_le_constraint(t_out_b, t_out_b_max * b_build, 'HP_Q_max')
        ...
        self.add_le_constraint(mflow_a, b_build * self.mdot_max,
                            'HP_mflow_a_des')
        self.add_input(identifier='IN_mflow_a', expr=mflow_a)
        
        self.add_output(identifier='OUT_mflow_a', expr=mflow_a)
        ...
        self.add_le_constraint(mflow_b, b_build * self.mdot_max,
                            'HP_mflow_b_des')
        self.add_input(identifier='IN_mflow_b', expr=mflow_b)
        
        self.add_output(identifier='OUT_mflow_b', expr=mflow_b)
        ...
        #######################################################################
        # Investment costs
        inv_costs = (spec_price * qdot_design +  fix_price) * b_build
        self.add_expression('investment_costs', inv_costs)
...
class Consumer(System):
    ...
    def __init__(self, label,**kwargs):
        ...
        # t_out_b_h=building_h['T_flow'],t_out_b_c=building_c['T_flow'])
